...
then
    echo "Бібліотека ffmpeg не знайдена. Будь ласка, встановіть її перед виконанням цього скрипту."
    exit
fi
...
if [[ "$option" == "1" ]]; then
    for file in *.mp4; do
        ...
        name="${file%.*}"
        mkdir "$name"
        ffmpeg -i "$file" -c:v libx264 -c:a aac -b:a 196k -start_number 0 -hls_time 2 -hls_list_size 0 -f hls "./$name/index.m3u8"
    done
elif [[ "$option" == "2" ]]; then
    for file in *.mp4; do
